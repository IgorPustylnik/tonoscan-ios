...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        window.window?.rootViewController = TabBarController()
        
        DispatchQueue.global().async {
            UserDataService.shared.loadDataFromServerToMemory() {
                DispatchQueue.main.async {
                    self.handleUserDataLoaded()
                }
            }
        }
    ...
    private func handleUserDataLoaded() {
        ...
        if userData.relativesPhoneNumber.isEmpty {
            self.window?.rootViewController = AddNumbersController()
        } else {
